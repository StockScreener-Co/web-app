...
import { Bookmark, Plus, ChevronRight, Loader2, Trash2 } from "lucide-react";
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <p className="text-muted-foreground max-w-md mb-8 leading-relaxed">
          To view and manage your watchlists, you need to sign in to your account.
        </p>
        <Link href="/auth">
    ...
    <div className="w-full max-w-[1200px] mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
        <div>
          <h1 className="text-4xl font-display font-extrabold tracking-tight mb-1 flex items-center gap-3">
            <Bookmark className="w-8 h-8 text-primary" /> My Watchlists
          </h1>
          <p className="text-muted-foreground">Track tickers and get value investing signals.</p>
        </div>
        <Button
          className="shadow-lg shadow-primary/20 rounded-xl"
          onClick={() => setCreateDialogOpen(true)}
        >
          <Plus className="w-4 h-4 mr-2" /> Create New Watchlist
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {watchlists.map((watchlist) => (
            <div key={watchlist.id} className="relative group">
              <Link
                href={`/watchlist?id=${watchlist.id}`}
                onClick={() => updateLastWatchlistId(watchlist.id)}
              >
                <Card className="hover:border-primary/50 transition-colors cursor-pointer group-hover:bg-accent/5">
                  <CardHeader className="pb-4">
                    ...
                    <CardTitle className="mt-4">{watchlist.name}</CardTitle>
                    <CardDescription>{watchlist.itemCount} ticker{watchlist.itemCount !== 1 ? "s" : ""}</CardDescription>
                  </CardHeader>
                  <CardContent>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        "{watchlist.name}" and all its entries.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
        ...
        <div className="text-center py-20 bg-muted/30 rounded-3xl border-2 border-dashed border-border">
          ...
          <h3 className="text-xl font-semibold mb-2">No watchlists found</h3>
          <p className="text-muted-foreground mb-8">You haven't created any watchlists yet.</p>
          <Button
            variant="outline"
            className="rounded-xl"
            onClick={() => setCreateDialogOpen(true)}
          >
            <Plus className="w-4 h-4 mr-2" /> Create Your First Watchlist
          </Button>
        </div>
      )}

      <CreateWatchlistDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} />
    </div>
  );
}
